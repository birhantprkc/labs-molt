...
A normal multi-turn episode is ONE segment with one action range per turn; a fresh segment starts on
a compaction (the agent rewrites/shortens history to fit context) or when the template re-renders
prior turns non-prefix (e.g. Qwen3 drops a prior turn's <think> block once a newer user query
follows). All segments share the one ``rollout_id`` + reward the trainer assigns (``stitch_session``).
...
async def _run_turn(state: ChatServerState, session: _Session, body: dict) -> tuple[str, str]:
    ...
    if extends:
        ...
        # The suffix slice below is only sound when the prior turns render the same in `full` and
        # `prefix`. A template can instead rewrite history in place — e.g. Qwen3 re-renders a prior
        # assistant turn WITHOUT its <think> block once a newer user query follows — and the slice
        # would silently drop or garble this turn's feedback. Treat that like a compaction.
        extends = full.startswith(prefix)
    if not extends:
        # New segment (episode start OR post-compaction/re-render): render the prompt in full +
        # tokenize (image count self-consistent with the vision encoder). process_prompt_with_images
        ...
        prompt_ids, mm_train_inputs, pil_images = await loop.run_in_executor(
            _TOKENIZE_EXECUTOR, _tokenize_observation, state.processor, full, pil_images
        )
        image_budget = (
        ...
        # Continuation: the new turn's tokens are the chat-template STRING suffix — the prior-turn
...
def stitch_session(state: ChatServerState, session_id: str, result):
    """Return the rollout's token-exact trajectory segments (usually ONE; more when the agent
    compacted history mid-episode or the template re-rendered prior turns non-prefix), each stamped
    with the terminal reward/score/info. Multi-turn

...
async def _run_turn(state: ChatServerState, session: _Session, body: dict) -> tuple[str, str]:
    ...
    if not extends:
        ...
        # new user/tool turn + generation prompt. Append it as masked feedback so the segment stays
        # token-exact; a small boundary-whitespace imperfection is fine (only marginal format).
    ...
    request_output, off_policy_len = await state.transport.generate(
        traj.observation_tokens, sp, multi_modal_data=mm_data, session_id=session.session_id
    )
